...
import 'package:aschente/screens/Practice/practice_arena.dart';
...
class QuestionsPreparations extends StatelessWidget {
  ...
  FutureBuilder<String> buildQuestions(BuildContext context) {
    return FutureBuilder(
      future: DefaultAssetBundle.of(context).loadString('assets/$subject.json'),
      builder: (BuildContext context, snapshot) {
        if (snapshot.data != null ||
            snapshot.connectionState == ConnectionState.done) {
          List _questions = json.decode(snapshot.data.toString());
          for (var i = 0; i < _questions.length; i++) {
            _pickedQuestions.add(_questions[i]);
          }
          Future.microtask(
            () => Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                builder: (_) => PracticeArena(questions: _pickedQuestions),
              ),
            ),
          );
        }
